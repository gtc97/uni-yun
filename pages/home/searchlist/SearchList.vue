	...
	<view style="width: 100%;">
		...
		<view class="cu-bar bg-white top-line min-height35">
			...
			<view style=" background-color: #ffdf40; border-radius: 8upx;">
				<picker mode="date" :value="date" start="2010-01-01" end="2050-12-31" @change="DateChange"
					style="padding-right: 0px">
					<view class="flex align-center">
						<view style="margin: 8upx">{{date}}</view>
						<view
							style="height: 22px; max-height: 22px border-left: 1px; border-right-style: solid; transform: scaleX(.333)">
						</view>
		...
		<view class="cu-modal bottom-modal" :class="modalName=='priceModal'?'show':''" @tap="hideModal">
			<view class="cu-dialog" @tap.stop="">
				<view class="padding-lr-sm">
					<view class="cu-bar top-line" v-for="(cond, index) in conds1" :key="index"
						:class="index==0 ? 'text-orange2' : ''">
						<view>{{cond.name}}</view>
		...
		<view class="cu-modal bottom-modal" :class="modalName=='faCheModal'?'show':''" @tap="hideModal">
			<view class="cu-dialog" @tap.stop="">
				...
				<view class="cu-bar bg-white top-line" v-for="(cond, index) in conds2" :key="index">
					...
					<view class="action cuIcon-square font-size21 "
						:class="index==0 ? 'text-orange2 cuIcon-squarecheck' : ''"></view>
				</view>
			</view>
		</view>
		<!--筛选模态框-->
		<view class="cu-modal bottom-modal" :class="modalName=='filterModal'?'show':''" @tap="hideModal">
			<view class="cu-dialog" @tap.stop="">
				...
				<view class="flex top-line" :style="{height: condHeight + 'px'}">
					<view class="flex-sub">
						<view class="cu-bar bottom-line" v-for="(cond, index) in conds" :key="index"
							@click="selCondTab(index)" :class="tabCur == index ? 'bg-white' : 'right-line'">
